...
- **Interactive host, no peer resolved** (host serving family un-attestable, no different-provider route installed, or disabled by config): one quiet line that the cross-model pass was skipped and why — name the config policy when that is the reason. Never an error.
...
- **Never started / not run** — the job was never started (gates not met, disabled by config, host un-attestable, no different-provider route installed, or CLI missing): the pass simply didn't run. Note "cross-model pass: not run" in Coverage for human-facing markdown — or "cross-model pass: disabled by config" when Step 1's egress policy was the reason; stay silent in `mode:agent`. Ignore any `*.raw.json` leftovers — they are not fold-in artifacts.
